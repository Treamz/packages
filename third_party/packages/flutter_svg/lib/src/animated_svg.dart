...
import '../svg.dart' show SvgErrorWidgetBuilder, SvgImageWidgetBuilder, SvgPicture, svg;
...
///
/// This is the animated counterpart to [SvgPicture], and takes the same
/// arguments for sizing, alignment, theming, and error handling. An SVG that
/// declares no animation renders exactly as [SvgPicture] would render it.
///
/// {@tool snippet}
///
...
/// expensive than for [SvgPicture], and the compiled frames occupy memory for
/// as long as they stay in [svg.animationCache]. Use [frameRate] and
...
class AnimatedSvgPicture extends StatefulWidget {
  ...
  ///
  /// See [SvgPicture.asset] for how `assetName` and `package` are resolved.
  ...
  ///
  /// Unlike [SvgPicture], this must be an [SvgLoader] rather than any
  /// [BytesLoader], because the animation is resolved from the markup itself
  /// rather than from an already compiled vector graphic.
  final SvgLoader<Object?> bytesLoader;
  ...
  /// providing a placeholder matters more here than it does for [SvgPicture].
...
class _AnimatedSvgPictureState extends State<AnimatedSvgPicture> with TickerProviderStateMixin {
  ...
  Future<void> _load() async {
    final SvgLoader<Object?> loader = widget.bytesLoader;
    ...
    try {
      ...
      final AnimatedSvgFrames frames = await svg.animationCache.putIfAbsent(
        key,
        () => loader
            ...
            .then(
              (SvgSource source) => compileAnimatedSvgFrames(
                ...
                colorMapper: source.colorMapper?.toVgColorMapper(),
  ...
  Widget _placeholder(BuildContext context) =>
      (widget.placeholderBuilder ?? SvgPicture.defaultPlaceholderBuilder)(context);
}
